...
- **Резолв роутера уходит внутрь туннеля** (XR-285) - провайдер отвечал на UDP-запрос поддельным NXDOMAIN с флагом `aa` даже «от имени» публичных резолверов, а прокси такой сайт не спасал: 53 и 853 выведены из перехвата, решение о проксировании принимается по SNI уже открытого TCP, и без адреса LAN-клиент TCP не откроет. Теперь dnsmasq спрашивает форвардер `xr-client` на `127.0.0.1:5353` (`xr-client/src/dns.rs`, секция `[dns]`), а тот открывает через туннель DoT-сессию с Quad9: провайдер видит соединение с адресом нашего же VPS и подменить в нём нечего. Апстрим держится одним соединением на все запросы, идентификаторы на проводе свои. Отката на резолвер провайдера нет: недоступный апстрим это SERVFAIL спрашивающему и строка в logread (первая сразу, дальше не чаще раза в минуту). Раскладку ставит `xr-setup` шагом `dnsmasq:tunnel` (`noresolv=1` и ровно один апстрим, соседний открытый адрес dnsmasq спрашивает наравне), порт живёт одной константой `render::DNS_FORWARDER_PORT`. Стенд с подложным резолвером в юнитах `xr-client/src/dns.rs`
...
- **Peek вне web-портов короткий, тишина уходит в Direct** (XR-292) - перехват заворачивает в xr-client весь TCP из LAN, и до решения о маршруте клиент подсматривает первые байты соединения. Раньше peek везде держал 10 секунд и по таймауту ронял соединение. Server-first протокол (VNC, SMTP, MySQL, IRC) на порту вне зашитого списка исключений молча висел и рвался. Снаружи это неотличимо от «сеть не работает», каждый случай лечился ручным пополнением bypass_rules. Решение о маршруте теперь считает `decide_route` в `xr-client/src/proxy.rs`. На 80/443 всё прежнее, peek ждёт SNI до 10 секунд и тишина рвёт соединение. Вне web-портов окно короткое (`PEEK_WAIT_OTHER`, 300 мс). Молчащий клиент по таймауту уходит в Direct с пустым буфером, соединение живёт и баннер сервера доходит до клиента. TLS-клиент с первым байтом 0x16 на нестандартном порту по-прежнему уезжает в Proxy. Обфускация под TLS шлёт ClientHello сразу после connect и в короткое окно укладывается. Зашитый список портов в redirect.rs остаётся оптимизацией без влияния на корректность. Тесты гоняют саму `decide_route`, `handle_connection` требует настоящего SO_ORIGINAL_DST.
...
- **Токены остаются в URL, маскирует каждый писатель лога** (XR-198) - инвайт-токен стоит в пути (`/invite/<токен>`, ссылку кодирует QR и ловит intent-фильтр), share-токен едет в query (`?token=`, ссылку открывает браузер, страница шары ходит с `Bearer`, query остаётся ссылкам скачивания). Убирать их из URL значит переписывать онбординг и дизайн публичного пути шар (LLD-38 п. 2.6). Поэтому инвариант «токены не логируем» держат писатели лога. Request-спан хаба берёт URI через `xr-hub/src/api/log_mask.rs`. Фронт nginx пишет `access_log` форматом `xr_masked` из `deploy/nginx-token-mask.conf` (`map` по `$request_uri` и `$http_referer`, тест `xr-hub/tests/nginx_token_mask.rs` гоняет регулярки файла без nginx), страница шары отдаётся с `Referrer-Policy: no-referrer`. Раскладка nginx остаётся ручной (LLD-13 п. 5.7), порядок в `docs/HUB-DEPLOY.md`, раздел 3. `error_log` при `502` пишет строку запроса как есть, это названный остаток
...
- `scripts/check-hub-log-mask.sh` - проверка маскировки токенов в собственном логе хаба живьём (XR-198): поднимает `xr-hub` из свежей сборки под `RUST_LOG=debug`, ходит по `/invite/<токен>` и по ручке с `?token=`, судит лог (токена нет, `<masked>` есть). Судится кодом возврата, последняя строка успеха `маскировка лога хаба: ок`
...
- `scripts/check-git-share.sh` - проверка git-контура шары живьём (XR-188 и XR-189, LLD-33 п. 6 фазы 1 и 2). Поднимает локальный хаб и агента из свежей сборки в temp-каталоге, клон, пуш с материализацией и fetch авто-коммитов идут штатным git. Первый пуш в пустовавшую при `--git` шару тоже входит в прогон. Сверяет лестницу гейтов, подпись HEAD с `agent_pubkey` гранта, long-poll и колпак `git_max_file_mb`. Последним шагом гоняет харнесс `xr-share sync` двумя машинами: клон, встречные правки и конфликт-копия, доезжающая до обеих сторон. Судится кодом возврата, последняя строка успеха `git-контур шары: ок`. Нужен git в PATH и OpenSSL 3 (на macOS берёт homebrew, системный LibreSSL не умеет ed25519)
- `scripts/check-share-web.sh` - проверка web-контура шары живьём (XR-190, LLD-33 п. 6 фаза 3). Поднимает локальный хаб и агента из свежей сборки в temp-каталоге, браузер это curl по ссылке с токеном в query. Сверяет отдачу страницы `/web` (наружу не ходит, без токена 401), закрытость истории read-токену (403), вывод `weblink` со ссылкой и предупреждением, `git/log` с путём в подпапке и `git/diff`, правку PUT'ом с If-Match до клона и 412 на протухшем хеше. Судится кодом возврата, последняя строка успеха `web-контур шары: ок`. Нужен git в PATH и OpenSSL 3, дерево сборки выводится от места самого скрипта, поэтому гонять можно из любого каталога
