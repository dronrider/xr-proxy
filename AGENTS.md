...
- `scripts/check-git-share.sh` - проверка git-контура шары живьём (XR-188, LLD-33 п. 6 фаза 1). Поднимает локальный хаб и агента из свежей сборки в temp-каталоге, клон, пуш с материализацией и fetch авто-коммитов идут штатным git. Сверяет лестницу гейтов, подпись HEAD с `agent_pubkey` гранта, long-poll и колпак `git_max_file_mb`. Судится кодом возврата, последняя строка успеха `git-контур шары: ок`. Нужен git в PATH и OpenSSL 3 (на macOS берёт homebrew, системный LibreSSL не умеет ed25519)
