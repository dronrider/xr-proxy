...
`http2` в этом блоке не включать, и это не забывчивость: WebSocket живой ленты
поднимается механикой `Connection: Upgrade`, а она есть только в HTTP/1.1. С
`listen 443 ssl http2` апгрейд превращается в обычный `GET`, и лента молча
...
Формат разбирается раньше ссылки на него, поэтому файл маскировки обязан
попасть к nginx до server-блоков. В раскладке с `sites-enabled` порядок такой
сам собой, а когда server-блоки лежат в том же `conf.d`, каталог читается по
алфавиту и файл ставится с именем впереди (`00-nginx-token-mask.conf`). Иначе
`nginx -t` отобьёт перезагрузку на неизвестном формате.
...
рабочий вариант это сертификат на конкретное имя публикации тем же способом,
каким живёт хаб:

```bash
certbot certonly --nginx -d demo.web.example.com
```

Тогда `server_name` в блоке ставится на это имя, а не на звёздочку, и на каждую
новую публикацию сертификат выпускается заново.
