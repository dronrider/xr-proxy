...
(по умолчанию `/var/backups/xr-hub`, каталог создаётся с правами 700, чтобы
даже листинг бэкапов не читался посторонними), имя несёт UTC-метку:
...
Не попадают `releases/` и `setup-dist/`: это раздача дистрибутивов, она
воспроизводится с машины сборки (`scripts/release-xr-setup.sh`, `sign-release`),
и гонять её десятки мегабайт каждую ночь незачем. `--keep N` (по умолчанию 14)
подчищает старые архивы, `--keep 0` отключает ротацию.

```bash
xr-hub backup --out /var/backups/xr-hub --keep 14
```

Команда печатает путь, размер, отпечаток ключа и счётчики: по выводу видно,
что уехало непустое.
...
2. Забрать свежий архив с приёмника и разложить:
   ...
   затираемые файлы остаются рядом копией `*.bak.<метка>`. Если на месте лежит
