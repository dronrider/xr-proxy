...
| XR-039 | Расцепить листинг и хеширование (агентная часть): manifest-эндпоинт отдаёт листинг без хеширования (метаданные + хеш из кеша, иначе пусто), SHA-256 досчитывает фоновый прогрев. Список открывается мгновенно даже на холодном кеше, прогрев больше не глохнет от конкурирующих запросов. Выкачено на оба хаба. Проверка: обновить агента, открыть большую шару сразу после рестарта -> список файлов появляется без зависания; скачивание не-прогретых файлов поднимается по мере прогрева (минуты). Остаток (опц.): мгновенное скачивание холодного файла через ленивый хеш на трансфере + diff синка по size+mtime | LLD | P1 | [xr-share/src/manifest.rs](../xr-share/src/manifest.rs), [xr-share/src/server.rs](../xr-share/src/server.rs) |
...
| XR-041 | Уведомление о новой версии в рамках всего приложения, а не только на главной (VPN). Сейчас баннер обновления виден лишь на главном экране. Сделать индикацию доступной с любой вкладки: глобальное состояние «доступно обновление» + бадж/точка на иконке нижней навигации. Переработать отображение, один из вариантов это свести настройки в одно место и показывать бадж на иконке настроек (дизайн открыт). Решить, на какой иконке/в каком разделе показывать | task | P2 | [xr-android/.../ui/update/UpdateBanner.kt](../xr-android/app/src/main/java/com/xrproxy/app/ui/update/UpdateBanner.kt), [UpdateManager.kt](../xr-android/app/src/main/java/com/xrproxy/app/update/UpdateManager.kt) |
| XR-042 | Логи не должны зависеть от состояния (сеть/режим), это просто журнал записей. Сейчас лента собирается из транзиентных источников (engine `recentErrors` + `probeLog`): при смене сети движок перезапускается и recentErrors обнуляется, после переключения доверенная -> обычная тянутся пробы доверенной. Сделать единый персистентный append-only журнал, переживающий перезапуск движка и смену сети/режима (не обнуляется): все источники пишут в один буфер. Само переключение сети/режима логировать отдельной записью. Ротацию (размер/кол-во) вынести в настройки | bug | P2 | [ui/VpnViewModel.kt:867](../xr-android/app/src/main/java/com/xrproxy/app/ui/VpnViewModel.kt#L867), [service/XrVpnService.kt](../xr-android/app/src/main/java/com/xrproxy/app/service/XrVpnService.kt) |
| XR-043 | Выбор директории хранения шары (фидбек пользователей: хотят выбирать). При первой синхронизации шары (первый тап по файлу, ручной синк или включение фонового синка) спрашивать: класть в директорию по умолчанию (app-dir) или выбрать свою; выбор запоминать на шару. Учесть Android scoped storage: нативный sync пишет по реальному FS-пути (std::fs), поэтому `getExternalFilesDir` работает, а SAF tree URI это НЕ реальный путь, движок туда писать не может (ровно поэтому ушли с SAF на app-dir, чтобы починить синк). В дизайне выбрать: ограничить реально-записываемыми путями, либо мост через DocumentFile (движок пишет во временный app-dir, Kotlin копирует в SAF, лишняя копия), либо MANAGE_EXTERNAL_STORAGE | task | P2 | [data/ShareRepository.kt:32](../xr-android/app/src/main/java/com/xrproxy/app/data/ShareRepository.kt#L32) |
