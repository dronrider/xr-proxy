...
| XR-037 | `xr-share install` перезатирает существующий конфиг: генерит новую identity и пишет пустой `shares`, поэтому повторный запуск установочной команды из панели теряет заведённые шары (агент начинает отдавать 404, на хабе шары висят осиротевшими под старым pubkey). Сохранять существующие `identity_key` и `shares` при наличии конфига, обновляя только мандат; перезапись только под `--force` | bug | P1 | [xr-share/src/cli.rs:21](../xr-share/src/cli.rs#L21) |
