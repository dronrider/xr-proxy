...
| XR-034 | Агент анонсирует приватный LAN-адрес (`192.168.x`), шара недоступна вне домашней сети. При `xr-share share` предупреждать, если резолвится приватный addr; задокументировать проброс `8443` + публичный IP/DDNS. Обдумать relay через хаб для агентов за NAT/CGNAT (агент звонит на хаб, хаб проксирует запросы потребителя) | task | P2 | [xr-share/src/cli.rs:47](../xr-share/src/cli.rs#L47) |
