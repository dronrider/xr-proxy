...
XR-029 (v2 мультишаринг): в работе. Стадия A (xr-proto: тип `AgentCredential` плюс
подпись и stateless-проверка мандата) реализуется. Дальше: хаб-эндпоинты
`exchange`/`add`/`mint`, мультишара-роутинг по `share_id` в агенте с горячей
перезагрузкой конфига, CLI `share`/`list`/`unshare`, `install` без привязки к
папке, затем пересборка и редеплой на оба хаба.
