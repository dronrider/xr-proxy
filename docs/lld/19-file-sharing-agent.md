...
SPA), `xr-core` (**движок one-way sync**: диф манифеста, что докачать/удалить —
...
(XR-025) — идентичность/реестр: запись шары живёт тем же стором; опц. heartbeat
...
**синхронизируют** выбранные файлы/директории на своё устройство (в MVP — Android).
...
- Десктоп-компонентов нет (даже macOS-клиент — P3 [XR-017], Windows нет нигде).
- Многопользовательской модели нет (один админ + анонимные клиенты, LLD-01); здесь
  доступ — по **подписанным токенам в доверенном кругу**, без аккаунтов.
...
### 2.2 Доступ — токен, проверяемый агентом
...
- Потребитель предъявляет токен **агенту**; агент верифицирует подпись **офлайн**
  pinned-ключом хаба (хаб **не в data-path**). Отзыв — TTL + опц. revocation-list.
...
- **Семантика удаления:** true mirror — если файл удалён на сервере, локальная копия
  удаляется (предупредить пользователя; «аддитивный» режим — follow-up, §7).
...
- **CGNAT/NAT без проброса в MVP не поддержан** — снаружи не видно. Релей через
  сервер (для CGNAT) — **отдельная задача** (§7) с компромиссом «транзит, не
...
- `xr-share` — Rust-бинарь под **Windows** (`x86_64-pc-windows-gnu`/msvc) и **Linux**
...
- Раздача **строго в пределах** `dir` (канонизация пути, отказ на `..`/симлинки
  наружу) — критично (§5.2).
- **client/sync-режим агента на десктопе** (миррор на Win/Linux) — **вне MVP**
  (потребитель sync = Android); см. §7.

## 3. Дизайн-решения

### 3.1 Хаб — индекс, не хостинг (юр-чистота)
...
### 3.2 Регистрация ручная (адрес+порт), heartbeat — опц. refinement

MVP: владелец сам вбивает `адрес:порт` (он же отвечает за достижимость). Heartbeat
агента (как роутер в LLD-17) — опциональное улучшение для online/offline и
динамического IP, не блокирует MVP.

### 3.3 Контроль доступа на агенте, подпись на хабе

Хаб — корень доверия (ed25519-ключ LLD-01) — подписывает токены; агент верифицирует
офлайн. Хаб вне data-path, доступ при этом настоящий (TTL/отзыв).

### 3.4 Sync — однонаправленный mirror, движок в xr-core

Read-only → mirror server→client без конфликтов (проще двунаправленной sync).
Логика дифа/плана — **чистый Rust в xr-core** (тестируемо, переиспользуемо), Android
...
десктопе — follow-up (§7), чтобы не раздувать MVP.

### 3.7 Идентичность агента — TOFU через хаб
...
3. **Mirror удаляет локальные файлы.** Удаление на сервере → удаление локальной
   ...
   — follow-up (§7). Удаление только в пределах синхронизируемого набора.
...
8. **Доступность зависит от машины пользователя / динамический IP.** Машина выключена
   → недоступна; ручной `адрес:порт` протухает при смене IP — статика/DDNS или опц.
   heartbeat (§2.1, §3.2).
9. **Юр-чистота при будущем релее.** Релей = байты транзитом; зафиксировать «транзит,
   не хранение», без логирования содержимого — компромисс отдельной задачи.
10. **Безопасность содержимого.** Доверенный круг + токены снижают абьюз; SHA-256
    даёт целостность, но не «безопасность» файлов — ответственность шарящего.

## 6. План проверки

Rust (подпись/traversal/диф/integrity) — автотестами; агент/приложение — ручная.
...
- **Аддитивный режим sync** (не удалять локально пропавшее на сервере) — follow-up.
- **Heartbeat агента** (online/offline + динамический IP) — refinement поверх ручной
  регистрации.
...
## 9. Универсальный мультишаринг (v2, доработка XR-028, в реализации)
