...
- **Одноразовость (опциональная)** — определяется флагом `one_time` при
  создании инвайта (LLD-01 §3.3). Сам scan/GET `InviteInfo` инвайт не
  сжигает; consume происходит на `POST /claim` (на нажатие Apply). Если
  `one_time` выставлен — повторный claim вернёт 410, приложение покажет
  ошибку. Если не выставлен — инвайт можно claim'ить многократно до
  истечения TTL (сценарий: один QR на семью/команду). Клиент этой
  логики не знает и доверять ей не должен — решение принимает сервер.
...
Отступление от исходного дизайна (XR-259): `http://` тоже принимается, но
только для приватного/loopback хоста или `localhost` (второй нужен под
`adb reverse`). Локальный стенд поднимает xr-hub без TLS на LAN-адресе, и без
этой ветки онбординг с него не проходил вовсе. Публичный хост по-прежнему
требует https, downgrade реального хаба так не пройдёт.
...
Онбординг разбит на два сетевых шага под реальный hub API
(LLD-01 §3.3):

- `GET /api/v1/invite/<token>` → `InviteInfo` (публичная метаинформация:
  preset, comment, status, expires_at). **Инвайт не consume'ится** —
  пользователю сначала показываем экран подтверждения.
- `POST /api/v1/invite/<token>/claim` → `InvitePayload` (полные
  туннельные параметры + hub_url). На этом шаге сервер списывает
  одноразовый инвайт (если `one_time`) и отдаёт секреты.

**Фаза 1 — `onInviteLinkReceived(raw)`** (после сканирования / вставки /
deep link):

```
1. Распарсить URL → (hub_url, token)
2. GET /api/v1/invite/<token>, timeout 5 сек
   - 200 + status="active" → ConfirmInvite(info)
   - 200 + status="consumed" → Snackbar «Приглашение уже использовано»
   - 200 + status="expired" → Snackbar «Приглашение истекло»
   - 404 → Snackbar «Приглашение не найдено»
   - сеть/TLS → Snackbar с соответствующей ошибкой
```

Отступление от исходного дизайна (XR-234): снекбары фазы 1 заменены
состоянием `InviteError`. Отказ живёт экраном с причиной, хостом хаба и
кнопкой «Повторить» (для сетевых ошибок), а не мелькает снекбаром со
сбросом на приветствие. Экран ожидания показывает «Проверяем приглашение»
и хост: адрес известен до сети, ссылка разбирается локально. Набросок
`Error(message)` ниже по тексту так и не был реализован в первой версии,
XR-234 фактически закрывает его.

**Фаза 2 — `applyInvite(hub_url, token, preset)`** (нажатие Apply на
ConfirmInvite):

```
1. POST /api/v1/invite/<token>/claim, timeout 5 сек → InvitePayload
   - 410 Gone → Snackbar «Приглашение уже использовано или истекло»,
     остаёмся на ConfirmInvite до Cancel
2. Сохранить туннельные параметры в prefs:
   - server_address, server_port
   - obfuscation_key, modifier, salt
3. Сохранить hub-секцию:
   - hub.url = payload.hub_url
   - hub.preset = payload.preset
4. TOFU public key:
   - GET /api/v1/public-key, timeout 3 сек
   - 200 → сохранить как trusted_public_key (base64)
   - 404 → подписи не используются, поле остаётся пустым
   - сетевая ошибка → WARN, trusted_public_key пустой; пользователь
     увидит Snackbar «хаб недоступен, подпись пресета не будет
     проверяться»
5. Первый фетч пресета (pre-warm cache):
   - GET /api/v1/presets/<preset>, timeout 5 сек
   - успех → кэш на диск (`hub_cache_dir/<name>.json`)
   - ошибка → WARN, пресет подтянется при первом Connect
6. Обновить VpnUiState и onboardingState → Completed
7. Навигация: ConfirmInvite → главный экран в состоянии Idle
```

Шаги 1, 4, 5 вместе не блокируют UI больше, чем суммарный per-request
timeout (дефолт 5+3+5=13 сек). Если хаб тупит — выходим с Snackbar и
всё равно завершаем Apply (секреты уже записаны на шаге 2-3). Пресет
тогда скачается при первом реальном Connect.

**Сетевые вызовы живут в `xr-core::onboarding`** (переиспользуют
`reqwest`-клиент и `PresetCache::save_to_disk`); JNI-обёртки в
`xr-android-jni` проброшены для Kotlin:

- `nativeParseInviteLink(raw) → JSON` — через `xr_proto::invite_url`,
  возвращает `{ kind, hub_url, token }` или `{ error }`.
- `nativeFetchInviteInfo(hub_url, token, timeout_ms) → JSON` — один GET
  на метаинформацию инвайта, возвращает `{ info: InviteInfo?, error? }`.
- `nativeApplyInvite(hub_url, token, preset, cache_dir, timeout_ms) →
  JSON` — бандлит Фазу 2, шаги 1/4/5: claim + public-key + preset-fetch.
  Возвращает `{ payload: InvitePayload?, public_key: String?,
  preset_cached: bool, errors: [String] }`. Одноразовый
  `tokio::runtime::Runtime` на вызов, чтобы не мешать основному движку.

### 3.7 Обновление пресета после Apply

После Apply в prefs лежат `hub.url`, `hub.preset` и (при успешном TOFU)
`trusted_public_key`. Дальше работает стандартный механизм обновлений из
[LLD-01 §6](01-control-plane.md) — отдельной Android-логики этот LLD не
вводит, только фиксирует, что именно включает Apply:

- **Forced-fetch при старте движка.** Каждый Connect дёргает
  `GET /api/v1/presets/<name>` с `If-None-Match`; 200 — кэш
  (`xr-core/presets/<name>.json`) перезаписывается, 304 — используется
  локальный. На сетевой ошибке стартуем с кэша, WARN в лог.
- **Фоновый sanity-check** раз в `refresh_interval_secs` (дефолт 5 мин)
  на уже установленном соединении — `GET /api/v1/presets` с
  `If-None-Match`; при свежей версии — скачивание и hot-swap живого
  движка (LLD-01 §6.2). Тихо, без UI-диалогов «доступно обновление».
- **Подпись.** Если `trusted_public_key` непустой — каждая новая версия
  проверяется по LLD-01 §3.5; несовпадение → WARN, версия отклоняется,
  продолжаем на прошлом кэше.
- **Видимость в UI.** Отдельного экрана нет; изменения активных правил
  прилетают в `VpnViewModel` через тот же binder-канал, что и остальные
  обновления состояния движка (LLD-02 §3).

**Частный случай: `trusted_public_key` пуст после Apply** (хаб был
недоступен в шаге 3.6.3, пользователь увидел соответствующий Snackbar).
Дальнейшие скачивания идут без проверки подписи — автоматической
догрузки ключа при следующем удачном ответе хаба не делаем намеренно,
чтобы не превращать «первый удачный response» в скрытый захват trust
anchor'а. Восстановление — повторное применение инвайта, когда хаб
доступен.

### 3.8 Обработка ошибок при применении
...
### 3.9 OnboardingState в ViewModel
...
### 3.10 Интеграция с xr-hub Admin UI
...
| `xr-core/src/onboarding.rs` (новый) | Логика хождения в хаб: `fetch_invite_info`, `apply_invite` (claim + public-key + preset-fetch + save to disk через `PresetCache::save_to_disk`). |
| `xr-core/src/lib.rs` | `pub mod onboarding;` |
| [xr-android-jni/src/lib.rs](../../xr-android-jni/src/lib.rs) | Три новые JNI-функции, все возвращают JSON-строку: `nativeParseInviteLink(raw)`, `nativeFetchInviteInfo(hub_url, token, timeout_ms)`, `nativeApplyInvite(hub_url, token, preset, cache_dir, timeout_ms)`. Последняя создаёт одноразовый `tokio::runtime::Runtime`, чтобы не мешать основному движку. |
| [NativeBridge.kt](../../xr-android/app/src/main/java/com/xrproxy/app/jni/NativeBridge.kt) | Объявить три соответствующих `external fun`. |
...
| [VpnViewModel.kt](../../xr-android/app/src/main/java/com/xrproxy/app/ui/VpnViewModel.kt) | `onboardingState: StateFlow<OnboardingState>`. `fun onInviteLinkReceived(raw: String)`. `fun onInviteConfirmed()`. `fun onInviteCancelled()`. `fun applyInvite(payload)`. Инициализация `onboardingState` в `init { }` (§3.9). |
...
3. **Двойное применение одного инвайта.** Consume происходит на сервере
   в момент `POST /claim` (шаг 1 фазы 2), т.е. на нажатие Apply. Если
   между scan и Apply кто-то другой успел нажать Apply первым, наш
   `claim` вернёт 410 — показываем Snackbar «Приглашение уже
   использовано», пользователь жмёт Cancel. Это корректно: сам scan
   инвайт не сжигает, так что скриншот QR, отправленный админу для
   проверки, не ломает onboarding у пользователя.
...
- **Мультипрофиль** — вынесен в [LLD-08](08-android-multi-server.md).
  LLD-08 переопределяет Apply: вместо перезаписи единственного
  подключения добавляет новый ServerProfile в список.
