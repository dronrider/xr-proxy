...
отличия и сценарий проверки в [XR-047](../tasks/archive/2026/XR-047.md).
...
Экран из двух явных секций (уточнено в XR-120). Сверху секция **«Серверные
правила»** с карточкой пресета (read-only), ниже секция **«Мои правила»**
(editable) с подписью «переопределяют серверные» и кнопкой **«+ Добавить
правило»**. Порядок применения читается по подаче, движок не меняем: мои
правила и так срабатывают раньше пресета. Секция серверных правил рассчитана
на несколько пресетов, чтобы подача не перестраивалась после XR-119. Когда VPN
на паузе в доверенной сети (`phase == Paused`), над секциями появляется
подпись, что трафик идёт напрямую и правила сейчас не действуют. В `TopAppBar`
иконка `{ }` «Show TOML», открывающая модальное окно с собранным конфигом.
...
- Выбор пресета (XR-119): имя пресета это дропдаун, тап открывает bottom
  sheet со списком пресетов хаба (`nativeListPresets` поверх публичного
  `GET /api/v1/presets`, поля name/version/rules_count). Выбор пишется в
  `hubPreset` профиля и применяется на следующем подключении; при живом
  туннеле показывается подсказка про это.
...
Scaffold
  ...
  content =
    LazyColumn:
      if (paused) item { RulesPausedNote() }
      item { SectionHeader("Серверные правила", "..."); PresetCard(preset, onRefresh, onDetails) }
      item { SectionHeader("Мои правила", "переопределяют серверные...") }
      items(userRules) { rule -> UserRuleRow(rule, ...) }
      item { AddRuleButton(onClick = { showAddDialog = true }) }
```

- `SectionHeader(title, subtitle)` это заголовок секции с поясняющей строкой (XR-120).
- `RulesPausedNote` это подпись при паузе в доверенной сети (XR-120), tertiary-tint.
