...
- `direct_reachable` ([sync.rs](../../xr-core/src/sync.rs), строка 1266) прощупывает
  агент неавторизованным `GET /{share_id}/manifest`: любой HTTP-ответ (даже `401`)
  считается признаком живости, транспортная ошибка или таймаут это недоступность.
  Ручка не идеальна для прощупа: она про данные и share_id, а живость это свойство
  хоста агента, share_id тут лишний.
- У агента уже есть неавторизованная ручка живости `GET /healthz`
  ([server.rs](../../xr-share/src/server.rs), строка 191): отдаёт голый `ok`, версии
  и статуса relay-аплинка не несёт, и прощуп её сейчас не использует.
...
- `direct_reachable` ([sync.rs](../../xr-core/src/sync.rs), строка 1266)
  переключается с `GET /{share_id}/manifest` на `GET /health`: чище, чем опираться
  на `401` от манифеста, и не требует share_id. Прощуп по-прежнему считает живостью
  любой валидный HTTP-ответ, тело не парсит (диагностические поля читает только
  ручная проверка и будущая админка).
...
Прощуп по `GET /manifest` держится на побочном факте «даже 401 значит жив» и тащит
лишний share_id в вопрос про живость хоста. Отдельная неавторизованная `/health`
чище: она агентовая, отвечает крошечным телом и заодно несёт версию и статус
relay-аплинка для диагностики. Расширяем уже существующую `/healthz`, а не плодим
третью ручку живости. Проверяет её консьюмер по прямому адресу: relay доступен
всегда, измерять надо ровно достижимость direct с места потребителя.
...
| `xr-core/src/sync.rs` | Параметр `mode: AccessMode` в `direct_then_relay` и прокинуть его через `sync_share_grant`/`upload_file`/`delete_file` и родственные; ветвление `direct`/`relay`/`auto`; `static VERDICT_CACHE` (per-agent, TTL) с лукапом и двойной инвалидацией; `direct_reachable` переводится на `GET /health`. |
...
- `test_direct_reachable_uses_health`: прощуп ходит на `/health`, живой агент
  доступен, транспортная ошибка/таймаут это недоступность (детерминированно, как
  существующий тест таймаута прощупа около строки 3167).
