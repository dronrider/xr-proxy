# LLD-37. Мгновенный подхват правил хаба: long-poll поверх цикла рефреша (XR-174)

**Статус:** Реализовано (XR-191). Отличия от дизайна два, оба в тестах:
`watch_loop` проверяется не на мок-часах, а событиями (мок-хаб на настоящих
сокетах плюс `tokio::time::pause` дают ложные срывы по таймауту, потому что
пауза перематывает часы, пока запрос идёт по сети), поэтому деградированная
ветка гоняется с `fallback_interval` в 20 мс и ждёт правил из канала; ручка
хаба тестируется не только через oneshot-роутер, публикация в тесте
пробуждения зовётся прямо `update_preset`, чтобы не поднимать админскую
сессию.
...
- `docs/ARCHITECTURE.md`: в описании хаба (раздел 4) появляется ручка
  ...
  на «long-poll с откатом в опрос»; строка LLD-37 в таблице раздела 9.
