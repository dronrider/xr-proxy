...
- **Статичный режим** (Idle, NeedsPermission, Paused, Stopping): щит без
  анимации, кольца не рисуются. Цвета: `primary` для щита, `background` для
  «пробоины». Молния целиком внутри силуэта, за края щита не выходит
...
- **Кольца вокруг щита** (уточнено в XR-186). Состояние несёт направление
  ...
  гаснет к концу пути.
- **Disconnected** это чуть приглушённые цвета (alpha 0.6 у всей композиции),
