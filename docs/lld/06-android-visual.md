...
- **Щит.** Мягкий силуэт (переработан в XR-233, итерация 2 по дизайн-доске):
  - симметричный, верхний край собран из двух дуг, плечи скруглены,
  - боковины плавные выпуклые,
  - низ круглый, а не остриём. Остриё спорило с кольцами вокруг иконки, а
    круглое дно повторяет их кривизну.
  - Геометрия живёт в квадратной системе 120 на 120, фигура стоит по
    оптическому центру (60, 61), а не по геометрическому: центр масс щита
    ниже середины бокса, и без поправки иконка кажется съехавшей вверх.
  - Заливка это вертикальный градиент со светом сверху: `#53E6F8` на 0,
    `#22D3EE` на 0.55, `#0E9FBF` на 1. Поверх плеч блик белым под 22%.
    Объём даём светом и формой, без глянца и растровых теней.
  - В моно-иконку уведомления сводится без потерь: там берётся только
    силуэт, градиент и блик отбрасываются.
...
- **Стрела-молния.** Пересекает щит по диагонали сверху-справа вниз-влево:
  ...
  - **Заливка** это `background` (#0B1220), то есть молния «вырезана» из
    щита как отверстие.
  - Молния целиком внутри силуэта, за края щита не выходит (уточнено в
    XR-186 и XR-187). Первоначальный бриф выпускал её концы наружу, но на
    128dp в приложении они читались как мусор по краям, а в иконке лаунчера
    выносили композицию за safe zone.

- **Background слой adaptive icon** это ровная заливка `background`
  (#0B1220), без градиента (уточнено в XR-187). Молния вырезана плоским
  цветом, и он обязан совпадать с тем, что лежит под ней: на градиенте
  «дырка» выше середины иконки заметно уходит в другой тон и читается как
  пятно, а не как отверстие.

**Что сделано и где лежит (уточнено в XR-187):**
- `res/mipmap-anydpi-v26/ic_launcher.xml` это adaptive icon: foreground
  ссылается на вектор `@drawable/ic_launcher_foreground`, background на
  `@color/background`.
- `res/drawable/ic_launcher_foreground.xml` держит щит с молнией в системе
  108x108. Геометрия та же, что рисует `ShieldArrowIcon`, только
  пересчитанная из системы 100x120: одна форма на лаунчер и на главный
  экран снимает риск из раздела 5, что два щита разъедутся.
- Силуэт вписан в safe zone (круг диаметром 66 в центре холста): самая
  дальняя точка контура это 32.15 от центра при допустимых 33. Прежний
  вектор был крупнее, верхние углы уходили на 43 и срезались круглыми
  масками.
- Растровых иконок лаунчера нет. Legacy PNG нужны были только для Android
  ниже 8.0, а minSdk у нас 29, поэтому adaptive-иконка применяется всегда,
  и `mipmap-*/ic_launcher.png` вместе с `mipmap-*/ic_launcher_foreground.png`
  удалены. До XR-187 adaptive-иконка ссылалась именно на растровый
  foreground, поэтому в лаунчере жил серо-стальной щит из чужой палитры,
  да ещё со своей круглой плашкой поверх фона иконки.
...
- **Статичный режим** (Idle, NeedsPermission, Paused, Stopping): щит без
  анимации, кольца не рисуются. Щит красим в `primary`, «пробоину» в `background`. Молния целиком внутри силуэта, за края щита не выходит
  (уточнено в XR-186; прежний набросок с выступающими концами не прижился,
  на 128dp они читались как мусор по краям).
- **Кольца вокруг щита** (уточнено в XR-186). Состояние несёт направление
  движения, а не скорость: в Connected три кольца расходятся от щита наружу
  (цикл 2400 мс), в Preparing / Connecting / Finalizing те же кольца сходятся
  к нему (цикл 1600 мс). Кольца это одна анимация со сдвигом старта на треть
  цикла, поэтому идут вереницей. Обводка 2dp цветом `primary`, прозрачность
  гаснет к концу пути.
  В Idle и NeedsPermission вся композиция идёт под alpha 0.6, это читается
  как «пока неактивно».
- **Системное «убрать анимацию»** (`ANIMATOR_DURATION_SCALE == 0`) выключает
  кольца совсем: бесконечное движение на главном экране это ровно то, от
  чего эта настройка избавляет.

Что было до XR-186: щит тянулся растром `xr_shield_icon.png`, а анимацией
было мигание alpha у одного кольца 3dp раз в 2 секунды. Растр разошёлся с
палитрой (серо-стальной глянец в круглой плашке вместо cyan-щита из 3.2), а
его плашка спорила с кольцом поверх. Теперь щит рисуется вектором на Canvas,
PNG удалён из ресурсов.
