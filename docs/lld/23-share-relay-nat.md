...
xr-server) и деплоится рядом с xr-server на существующих VPS отдельным
systemd-юнитом и портом.
...
## 9. Открытые вопросы (владельцу на утверждение)
...
- *Сигналинг?* -> **хаб, relay-дескриптор и relay-токен в грантах** (§3.6,
  §3.7).
- Деплой: relay на **обоих** VPS сразу или сначала на одном (primary)?
  Дизайн не зависит, вопрос только объёма первой выкатки.
