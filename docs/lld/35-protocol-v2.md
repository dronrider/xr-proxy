...
- **UDP relay.** Датаграммный путь ([udp_relay.rs](../../xr-proto/src/udp_relay.rs))
  ...
  Хендшейк и шифр общие с TCP-путём, дисциплина nonce разная. Это же
  подготавливает почву под XR-064 (QUIC/UDP). Деталь датаграммной крипты
  специфицирует срез под XR-061/XR-064.
...
- [ARCHITECTURE.md](../ARCHITECTURE.md) раздел **5.1**: описание TCP-туннеля
  ...
  nonce. Раздел **4.1**: `obfuscation.rs` уходит из data-path (после ретайра
