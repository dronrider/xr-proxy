...
вместо локального, когда хаб знает web-домен. Публичный URL проходит через
nginx, и его `access_log` обязан идти форматом `xr_masked`
(`deploy/nginx-token-mask.conf`, XR-198). Стоит рассмотреть токен во
фрагменте (`/web#token=`) вместо query. Страница-оболочка тогда отдаётся без
гейта, JS берёт токен из `location.hash`, и фронт не видит его даже на первом
заходе. Ссылкам скачивания токен по-прежнему нужен, им остаётся query.
