...
с устройства: создавать директории любой вложенности, перемещать и копировать
файлы между ними, перемещать и переименовывать директории. Модель доверия не
...
- `POST /{share_id}/copy`, тело `{from, to}`: копия **файла** (`fs::copy` в
  `spawn_blocking`). 201 при успехе, 404 если `from` нет, 409 если `from`
  это директория или `to` занят. Рекурсивная копия директории дорогая,
  прогресс стримить некуда, и запроса на неё нет.
...
- copy сеет цель хешем источника (`hash_of(from)`: тёплый кеш отдаёт без
  чтения, холодный хеширует один раз) с метаданными свежей копии.
...
- `xr-core`: `mkdir(grant, path, timeout)`, `move_entry(grant, from, to,
  timeout)`, `copy_file(grant, from, to, timeout)` в write-блоке `sync.rs`
...
- Приложение (контролы видны только при `share:write` в гранте, новый
  ...
  - long-press по папке (сейчас не занят): те же действия без «Копировать»;
  - «Переместить» и «Копировать» открывают пикер целевой папки: диалог по
    дереву манифеста (тот же `explorerLevel` поверх `dirs` + префиксов,
    навигация с крошками), подтверждение кнопкой «Сюда»;
...
манифест единственным источником структуры; цена это по строке пути на
директорию, на реальных шарах пренебрежимо.
...
| `xr-share/src/server.rs` | Роуты `POST /{id}/mkdir`, `POST /{id}/move`, `POST /{id}/copy` (v2-only); общий гейт как у `handle_put`; тела операций (`create_dir_all`, занятость `to`, запрет move внутрь себя, случай смены регистра, `fs::copy` в `spawn_blocking`); пересев/перевес хеш-кеша; логирование операций (пути и исход, без токена). |
...
- `server.rs`: `test_mkdir_creates_nested` (201, видна в манифесте, повторный
  ...
  (201, sha256 цели совпадает с источником сразу; источник-директория 409,
  занятый `to` 409).
- `xr-core`: `plan_mirrors_empty_dirs` (mkdir/rmdir в плане с selection и
  ...
  желаемую пустую директорию; `mkdir`/`move_entry`/`copy_file` против
...
8. **Старый потребитель против нового агента** не видит пустых директорий и
...
2. `xr-share mv --invite <t> --share <имя> report.pdf "docs/новая/report.pdf"`
   ...
   с прежним sha256.
3. `mv` директории целиком и `cp` файла: содержимое и хеши на местах; `mv` в
   занятый путь -> отказ с текстом про 409; `mv` директории внутрь самой
   себя -> отказ.
...
- **Рекурсивная копия директории**: вернёмся при живой боли, с прогрессом.
...
- *Copy директории?* -> **нет**, только файл (п. 2.2).
