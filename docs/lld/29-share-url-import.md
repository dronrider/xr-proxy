...
**Статус:** Реализовано (XR-141). Отличия от дизайна косметические: у
`max_height` появился default 1080 (блок без поля валиден), в JNI высота
кодируется значением `<= 0` как «не задана», сабминутные таймауты в тестах
крутятся напрямую через `JobSpec`.
...
args = [
  ...
  # Источник файла (XR-255): строка на файл в .xr-meta.tsv джоб-папки.
  "--print-to-file",
  "after_move:%(filepath)s\t%(webpage_url)s\t%(channel,uploader|)s\t%(channel_url,uploader_url|)s\t%(upload_date|)s\t%(title|)s",
  ".xr-meta.tsv",
  "{url}",
]
```
...
Источник файла (опционально, XR-255): файл `.xr-meta.tsv` в cwd, строка на
выходной файл, поля через табуляцию (`<файл>`, `<url страницы>`, `<источник>`,
`<адрес источника>`, `<дата>`, `<заголовок>`). Имя файла берётся из первого
поля, путь в нём допустим. Битые строки агент пропускает, отсутствие файла это
не отказ импорта: у публикации всё равно остаётся ссылка самой джобы.
Референсный yt-dlp пишет их через `--print-to-file`, а не `--print`, потому что
второй тянет за собой `--quiet` и погасил бы прогресс.
...
- Android UI (минимум контролов, тексты фиксируются здесь): в папке шары,
  ...
  прочее как есть).
- Сорванный импорт показывается диалогом, а не тостом (XR-161, правка после
  реализации): заголовок «Импорт не удался», в теле текст агента моноширинным
  со скроллом, кнопки «Скопировать» (кладёт причину в буфер целиком, тост
  «Скопировано») и «Закрыть». Тост тут не годился: с Android 12 в нём две
  строки и полторы секунды, а агент присылает хвост stderr плагина, ради
  которого его и завели. Диалог живёт на верхнем уровне экрана файлов, потому
  что джоба доезжает и после ухода из папки. Короткие сообщения импорта
  («Импорт завершён», «Импорт уже идёт, дождись или отмени его») остаются
  тостами.
