...
sequenceDiagram
    ...
    User->>Agent: POST /{share_id}/import {url, dest}
...
каждый публикуемый файл. Права выводятся из права записи (п. 2.2), результат
...
- `POST /{share_id}/import`, тело `{"url": "...", "dest": "видео/сериалы"}`
  (`dest` это каталог внутри шары относительно корня, пустая строка это
  корень). Ответ `202 {"job_id": "..."}`; id случайный, 8 байт hex.
...
- `xr-core`: `import_url(grant, url, dest) -> job_id`,
...
- Android UI (минимум контролов, тексты фиксируются здесь): в папке шары,
  ...
  «Импорт по URL». Диалог: поле «Ссылка», кнопки «Импортировать» и
  «Отмена». После запуска в списке файлов сверху строка задачи: «Импорт:
...
- Харнесс на десктопе: `xr-share import --invite <t> --share <id|имя> <url>
  [--to <rel>]` запускает джобу и поллит до конца, печатая прогресс; это
...
| `xr-share/src/cli.rs` | Флаг `share --import` (только с `--writable`, повторный `share` без флага выключает); субкоманда `import` (запуск + поллинг с печатью прогресса); тексты `--help`. |
| `xr-core/src/sync.rs` | `import_url` / `import_status` / `import_cancel` поверх `direct_then_relay`; проверка `share:import` в скоупе гранта до сети. |
| `xr-android-jni/src/lib.rs` | `nativeImportUrl` / `nativeImportStatus` / `nativeImportCancel`. |
| `xr-android` (files UI) | Действие «Импорт по URL» в папке шары с `share:import` в гранте; диалог ссылки; строка прогресса с отменой и опросом раз в 2 с; обновление листинга по done; тексты из п. 2.8. |
...
| `configs/share.toml` | Блоки `[import]` и `[[import.plugin]]` с рабочим yt-dlp-примером и комментариями (включая `--no-playlist` и почему), `import` у `[[share]]`. |
...
4. Ссылка на страницу без видео (или с опечаткой) -> джоба failed с
   внятным текстом из stderr yt-dlp.
5. URL с приватным адресом (например админка роутера) -> 400 сразу; ссылка
   с редиректом на приватный адрес на Linux-агенте -> джоба failed
   (песочница режет соединение).
6. Инвайт без write-привязки: действия импорта в UI нет, харнесс
   отказывает до сети текстом «нет права импорта»; токен с
   `share:read share:write` без import (сминченный руками) по POST -> 403.
7. Шара без `--import` -> 403; конфиг без плагинов -> 403.
8. Отмена: крестик на строке прогресса (и `DELETE` харнесса) убивает
   скачивание, мусора в шаре и `.xr-import-*` не остаётся.
9. Рестарт агента посреди импорта: temp подметён, телефон показывает
   «задание потерялось», повторный импорт проходит.
10. Relay-путь: тот же импорт с телефона на шаре за NAT.
11. Дока сходится с поведением: README агента описывает контракт плагина и
    гейты, `configs/share.toml` содержит рабочий референс yt-dlp.
12. `cargo test --workspace` зелёный, 0 warnings.
...
- **Импорт с прямым выбором формата/качества** (меню «1080p / аудио»):
  сегодня выбор фиксируется конфигом плагина у владельца; параметризация
  джобы с устройства это отдельная задача поверх.
