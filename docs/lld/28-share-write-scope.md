...
5. **Утечка токена с write-скоупом.** Тот же класс, что у read: TTL 7 дней, не
   ...
   даёт применить его к другой шаре. В `?token=` браузерной ссылки
   write-скоуп попадает. `xr-share weblink` и экран файлов приложения
   печатают именно write-токен для страницы правки (LLD-33 п. 2.8, риск 6
   там же), фронт и хаб такой URL в логах маскируют (XR-198).
