...
переводит dnsmasq на Quad9 и запускает сервис. По `--enroll-token` роутер
регистрируется в реестре хаба (LLD-17; серверная часть реестра это XR-025).
Смена SSID применяется последним шагом с задержкой, чтобы не оборвать
Wi-Fi-сессию, через которую идёт настройка.
