...
Ручные шаги ниже сведены в идемпотентный установщик (LLD-13, XR-177). На
роутере с доступом в интернет достаточно:

```sh
curl -fsSL https://<хаб>/api/v1/setup/install.sh | sh -s -- router \
  --server <addr:port> --key <b64> --salt <0xHEX> \
  --hub-url https://<хаб> [--enroll-token <t>] [--ssid HomeNet --wifi-pass <psk>]
```

Ключ и salt печатает установка сервера (`xr-setup server`); `--server` можно
повторять, порядок задаёт приоритет пула (LLD-10). Установщик кладёт
`xr-client` с раздачи `setup-dist`, генерирует `config.toml` (пул
`[[servers]]`, auto_redirect, пресет хаба), ставит procd init с fd-limit и
watchdog-cron, nftables-скрипты (kill-switch, UDP TPROXY), sysctl-буферы,
переводит dnsmasq на локальный DNS-форвардер клиента (XR-285, резолв уходит
внутрь туннеля по DoT, шаг `dnsmasq:tunnel`) и запускает сервис. По
`--enroll-token` роутер регистрируется в реестре хаба (LLD-17; серверная
часть реестра это XR-025). Смена SSID применяется последним шагом с
задержкой, чтобы не оборвать Wi-Fi-сессию, через которую идёт настройка.

Повторный запуск идемпотентен: готовые шаги отчитываются «уже настроено»,
существующий конфиг без `--force` не перетирается. Финальная проверка
убеждается, что процесс жив, таблица `ip xr_proxy` стоит и DNS отвечает.

Дальше по разделам - тот же путь руками (нужен для нестандартных арок,
разбора инцидентов и понимания, что именно ставит установщик).
...
Это маловероятно (~4-8 МБ RAM при активной работе), но если случилось:
...
### Просмотр причин падений

Watchdog записывает диагностику каждого краша в персистентный лог:

```bash
cat /etc/xr-proxy/crash.log
```

Лог содержит: время краша, вывод dmesg (OOM-killer), последние записи xr-client и состояние памяти. Файл автоматически обрезается до ~50 КБ.
...
/usr/bin/xr-watchdog.sh         # watchdog (автоперезапуск + очистка правил)
...
| **Watchdog (cron)**        | Каждую минуту проверяет, жив ли процесс xr-client. Если нет — перезапускает и пишет в crash.log | Процесс восстанавливается в течение 1 минуты, причина краша сохраняется |
