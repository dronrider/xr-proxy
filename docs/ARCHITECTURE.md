...
8. Соединение держится до явного отключения пользователем (XR-279).
   ...
   Гасит флаг только `stopFromUi` (кнопка и уведомление), системный отзыв
   `onRevoke` оставляет (различение XR-221). Три события, раньше убивавшие
