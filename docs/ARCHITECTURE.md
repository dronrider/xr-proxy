...
файла системному вьюеру, и снятие локальной копии её не отменяет. Строка файла
показывает дату из `mtime` манифеста (когда файл появился у агента) коротким
системным форматом. Хвост ютуб-импорта (`[<11 символов base64url>]` перед
...
приложение (XR-250), где удаление из шары живёт в карточке файла по долгому
нажатию, показывается только при `share:write` в скоупе токена и спрашивает
