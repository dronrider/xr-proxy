...
| 17 | [17-hub-router-registry.md](lld/17-hub-router-registry.md) | Хаб-реестр роутеров + удалённое управление: идентичность/enrollment роутера, **исходящий** control-канал роутер→хаб (pull: отчёт статуса + приём подписанных команд из закрытого enum), раздел «Роутеры» в админке. Несёт «последний снимок» статуса для админки (история/Grafana — LLD-18). Шов с LLD-13: bootstrap регистрирует роутер. | Шаги 2 (LLD-01), 10 (LLD-10), 11 (LLD-11), 13 (LLD-13) | Draft |
