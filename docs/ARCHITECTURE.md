...
| [xr-web/](../xr-web/) | Браузерный вход к публикациям агентов (LLD-38 фаза 2, XR-263): адресация по поддомену, вход владельца по cookie-сессии, кеш маршрутов, пул соединений до агента, проксирование HTTP. |
...
**Чего здесь ещё нет.** Апгрейд до WebSocket это фаза 3 (XR-264), страница шары
наружу фаза 4 (XR-265). Апгрейд на агенте пока честно не пробрасывается:
`Upgrade` и `Connection` снимаются как hop-by-hop, полусплайс отдавать хуже, чем
не отдавать ничего.

### 4.10 xr-web: браузерный вход владельца (LLD-38 фаза 2, XR-263)
...
в памяти VPS. Апгрейд (фаза 3) заберёт аренду насовсем, для этого у неё есть
`detach`. До агента запрос едет как пришёл (метод, путь, query, тело потоком)
...
отказ мандата, обрыв туннеля и строку `метод-статус-длительность`; тела,
query-строки и cookie в лог не попадают.
...
в неё в последний момент, не пропал вместе с потоком. Отдельного сборщика
