...
| 28 | [28-share-write-scope.md](lld/28-share-write-scope.md) | Доступ к шаре на запись (XR-051): отдельный write-токен хаба со своим доменом подписи (read-путь не трогается), право записи у привязки шара-инвайт, двойной опт-ин владельца (writable в записи хаба и в конфиге агента), приём `PUT`/`DELETE` агентом строго в пределах шары (safepath, атомарная заливка temp + rename, хеш на лету), харнесс `push`/`rm`. Фундамент XR-052 (импорт по URL) и любых правок шары с устройства. | LLD-19, LLD-23; стык с LLD-27 | Draft |
