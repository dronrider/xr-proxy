...
relay-мост. Неймспейс `.xr-*` и собственный `.git` в историю не едут. Файлы сверх колпака
тоже. Колпак у харнесса это общая константа `GIT_MAX_FILE_MB` из xr-proto.
Настройку `git_max_file_mb` конкретного агента он не видит. Грант её не несёт,
и новых полей git-контур ему не добавляет (LLD-33 п. 2.1). Сдвинувший колпак
владелец расходится с соавторами на файлах между двумя значениями. Это
записано ограничением в README агента. Отвергнутый push это
...
не попадают. Фронт nginx перед хабом и `xr-web` пишет тот же URL, поэтому его
`access_log` идёт форматом `xr_masked` из `deploy/nginx-token-mask.conf`.
`/invite/<токен>` и `token=` в запросе и в `Referer` ложатся как `<masked>`, а
request-спан самого хаба берёт URI через `xr-hub/src/api/log_mask.rs` с теми
же заменами (XR-198). Токены остаются в URL по замыслу, ссылку открывает
браузер; маскирует каждый, кто URL пишет в лог. Живость публикации смотрится не в логе, а
