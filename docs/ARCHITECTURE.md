...
**Чего здесь ещё нет.** Браузер сюда пока не ходит: HTTPS-фронт `xr-web` с
входом владельца, поддоменами и пулом соединений это фаза 2 (XR-263), апгрейд до
WebSocket с живучестью фаза 3 (XR-264), страница шары наружу фаза 4. Апгрейд на
агенте пока честно не пробрасывается: `Upgrade` и `Connection` снимаются как
hop-by-hop, полусплайс отдавать хуже, чем не отдавать ничего.
...
- `POST /api/v1/expose/:name/mandate` это мандат публикации (`ExposeToken`) на свою публикацию: им проверяют путь харнессом `expose open`, а в фазе 2 маршрут будет собирать фронт служебной ручкой под общим секретом.
