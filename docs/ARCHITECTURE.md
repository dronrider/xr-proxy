...
- [redirect.rs](../xr-client/src/redirect.rs) — управление nftables/iptables
  ...
  Каждый VPS пула выводится из перехвата не целиком, а двумя правилами:
  туннельный порт (телефону с персональным клиентом в LAN нельзя получить
  туннель в туннеле) и всё, кроме 80 и 443 (ssh и служебные порты сервера).
