...
- [redirect.rs](../xr-client/src/redirect.rs) — управление nftables/iptables
  ...
  общего redirect. Условие с чужим вердиктом, кавычкой или разделителем
  команд отбраковывается с WARN, а если `nft` не принял набор целиком, клиент
  ставит перехват без машинных условий и говорит об этом в лог: опечатка в
  конфиге не должна оставлять LAN без перехвата вовсе.
