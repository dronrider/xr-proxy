...
- В [sync.rs](../xr-core/src/sync.rs) `direct_then_relay` перебирает прямые
  ...
  коротким liveness-probe при наличии запасного пути, первый ответивший
  обслуживает операцию. Мёртвый LAN-IP снаружи отсекается за секунды, а не за
  полный таймаут. Неответ или подмена (подпись манифеста не сошлась, значит по
  адресу отвечает чужой хост) уводит на следующий адрес; авторитетный ответ
  агента (HTTP-статус, протухший токен) возвращается сразу. Relay (pinned-TLS
  поверх loopback-forwarder) поднимается, только когда исчерпаны все прямые.
...
| 34 | [34-android-reconnect.md](lld/34-android-reconnect.md) | Переподключение Android-клиента (XR-132): новая фаза `Reconnecting` в `Phase`/`ConnectPhase` (долгоживущая, трафик напрямую, сервис держит foreground без `stopSelf`), флаг намерения `stayUp` (взводится нативным `Connected` и авто-подъёмом, снимается только пользовательским стопом), вечный авто-ретрай с backoff 5с -> 5мин, пинки по реальной смене сети и screen-on, без таймера в мёртвой зоне `noNetwork`. Ручной Connect при провале остаётся Error -> Idle (асимметрия). Только `xr-android`, `xr-core`/JNI не трогаются. | LLD-02, LLD-15; стык с XR-095, XR-183, XR-049 | Draft |
