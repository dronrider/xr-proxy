...
follow-up) → **LLD-07** (per-app, по ситуации). Номера шагов в таблице —
историческая нумерация (порядок появления LLD); фактическую очередь задаёт этот
список и колонка «Зависит от».
