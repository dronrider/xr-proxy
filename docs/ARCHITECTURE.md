...
битый CIDR. Fail-soft здесь принципиален, потому что пресет общий на весь парк
и одна опечатка не должна ни ронять клиента, ни лишать его маршрутизации
(XR-206: `*яндекс.рф` резался по байтам внутри многобайтового символа и валил
процесс на первом же SNI).
