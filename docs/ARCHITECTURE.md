...
bearer: libgit2 и штатный git шлют креды только после такого вызова, а пароль
...
папки он сам, прятать нечего. Пустая или несуществующая папка клонируется,
пустовавшая шара клонируется как `init` плюс origin (веток ещё нет, и первый
push соавтора создаёт `main`), непустая посторонняя папка это отказ с
названной причиной.
...
блокирующий нарочно: git2 синхронен, рантайм заводится только под relay-мост.
Колпак размера и неймспейс `.xr-*` те же, что у агента (константа
`GIT_MAX_FILE_MB` в xr-proto), плюс собственный `.git`. Отвергнутый push это не
ошибка, а сигнал повторить fetch-merge-push: так лечится и гонка двух
пишущих, и грязная папка владельца, чей pre-receive отбил материализацию.
Протухший за неделю токен перезапрашивается по инвайту, пять неудачных
проходов подряд перевыбирают путь до агента.

Слияние живёт здесь, а не у агента. Fast-forward выставляет рабочую копию,
...
версией, встречная кладётся рядом целым файлом `<имя> (конфликт <автор>
...
подписью, сходящейся с `agent_pubkey` гранта: это и проверка достижимости, и
anti-wrong-host. После fetch приехавший `main` сверяется с подписанным HEAD,
...
знает, а пиннинг остаётся в одном месте, в нашем rustls-коде. Это зеркало
