...
| 32 | [32-share-structure.md](lld/32-share-structure.md) | Структурирование шары с устройства (XR-168): директории как сущности манифеста (`ShareManifest.dirs`, все, включая пустые; подпись кроет байты тела и не меняется), эндпоинты агента `POST /{id}/mkdir`, `/move`, `/copy` под тем же `share:write` (гейты как у PUT, занятая цель 409 без перезаписи, move работает для файла и директории, copy только файла), пересев хеш-кеша при move, зеркало пустых директорий в синке (`SyncPlan.mkdir`/`rmdir`, прунинг становится desired-aware), клиент mkdir/move/copy в xr-core + JNI + приложение (создание папки, перенос/копирование/переименование с пикером целевой папки, гейт `canWrite`), харнесс `mkdir`/`mv`/`cp`. Хаб и минтинг не трогаются. Оптимизация синка под серверный move это XR-169. | LLD-28 (XR-139), LLD-19, LLD-23, LLD-29 | Draft |
