...
получатель по той же ссылке придёт за payload'ом ещё раз. Тело ответа при этом
обезличено (ручка публичная, устройство каталогов хаба постороннему ни к чему),
а причина уходит в лог строкой `не сохранилось потребление инвайта` с путём, на
котором споткнулась запись; путь в ошибку кладёт сам `storage`, там каждая
io-ошибка сохранения обёрнута `context` с именем каталога или файла.
