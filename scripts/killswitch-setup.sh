...
# Однострочный массив обязан закрываться на своей же строке: у sed-диапазона
# конец ищется только со следующей, и до ближайшей ] в кандидаты уезжали
# кавычённые значения соседних ключей (так на живой машине киллсвитч пробовал
# ставить условиями 'block' и 'debug').
BYPASS_RULES=$(awk '/^bypass_rules[[:space:]]*=/ { grab=1 }
    grab { print; if (index($0, "]")) grab=0 }' "$CONFIG" 2>/dev/null \
    | grep -oE '"[^"]*"' | tr -d '"')
