...
echo "=== Проверка ==="
sleep 2
# Конфиг судит сам бинарь (XR-227): причина отказа видна в выводе деплоя,
# а pgrep ниже по-прежнему отличает живой процесс от упавшего.
if $SSH_CMD "${REMOTE_BIN} validate -c /etc/xr-proxy/config.toml"; then
    echo "OK: конфиг прошёл сухую проверку"
else
    echo "ОШИБКА: конфиг не прошёл проверку, причина выше" >&2
fi
