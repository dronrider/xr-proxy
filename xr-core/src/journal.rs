...
//!
//! Стиль самого `message` общий для всех источников, иначе лента распадается на
//! куски по автору записи: одно событие это одна строка по-русски со строчной
//! буквы, тема и детали через двоеточие и запятые (`проба telegram.org, попытка
//! 1 из 2: недоступен, TimeoutException`). Своей разметки сообщение не несёт:
//! рамки, отступы и значки ломают и поиск по ленте, и выравнивание, а уровень с
//! источником журнал показывает сам. Английскими остаются только вложенные
//! тексты ошибок от системы и developer-логи `tracing`, их читает разработчик.
//!
//! Исключение по заглавной букве это строки двойного назначения: та же фраза,
//! что легла в журнал, уходит пользователю в интерфейс (снекбар, `VpnState::
//! Error`), и там заглавная на месте. Плодить ради регистра вторую формулировку
//! того же события не надо.
