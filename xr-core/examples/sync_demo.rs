...
//!
//! ```sh
//! cargo run -p xr-core --example sync_demo -- <agent_url> <dest_dir> <token.json>
//! ```
//!
...
async fn main() {
    ...
    if args.len() != 4 {
        eprintln!("usage: sync_demo <agent_url> <dest_dir> <token.json>");
        std::process::exit(2);
    }
    ...
    let manifest = fetch_manifest(agent_url, &token, timeout)
        .await
