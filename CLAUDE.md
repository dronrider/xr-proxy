...
- **UDP relay сервера: bind(src_port) резервируется до самого bind, не после** (XR-200) - на первый пакет нового src_port уходит `PortState::Binding(Notify)` в таблицу сразу, а сам bind выполняется уже без блокировки. Параллельный второй пакет того же src_port застаёт резервацию и ждёт готовый сокет через `Notify` вместо своего bind: обратный порядок (bind, потом insert) даёт гонку, где два первых пакета поднимают два сокета и второй insert затирает `BoundPort` первого, а тот вместе со своим receiver-таском остаётся висеть до idle timeout.
