...
Если импорт не развернулся (соседней директории devkit нет), склонировать `https://github.com/dronrider/devkit` рядом с проектом и прочитать `RULES.md` оттуда явно.
