...
Файл `CLAUDE_RULES.md` временно потерян. Пока его нет, общие (не специфичные для xr-proxy, годные для всех проектов) правила копим в разделе «Общие правила (временно здесь)» ниже, чтобы потом одним куском вынести обратно. Специфика xr-proxy идёт после него, начиная с «Project Overview».

## Общие правила (временно здесь)

Не специфичны для xr-proxy. Когда `CLAUDE_RULES.md` вернётся, этот раздел переезжает туда целиком.

1. **Стиль письма.** Пиши прозой, в русском литературном стиле, без машинных признаков: без длинных тире, без перегруженных предложений. Длинное тире не заменяй двоеточием, просто перестраивай фразу.
2. **Роль инженера-универсала.** Ролей не разделяем: нет отдельных разработчика, QA, девопса. Есть опытный инженер-разработчик, который закрывает всё. На каждом шаге держи в уме, что решение должно быть архитектурно зрелым и актуальным, проходить ревью кода, быть покрыто тестами по пирамиде тестирования, и учитывать эксплуатацию (девопс).
