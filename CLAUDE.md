...
- **UDP relay сервера: таск с очередью на каждый поток, приём ничего не ждёт** (XR-200) - пакет из туннеля кладётся под локом таблицы в очередь своего `src_port`, и цикл приёма сразу идёт за следующим, а bind нового сокета выполняет таск потока. Иначе первый пакет нового src_port останавливает relay целиком на время bind. Сокетом владеет ровно один таск, он же снимает слот из таблицы (по неудачному bind сразу, по простою с перепроверкой очереди под тем же локом), поэтому второго сокета на тот же src_port не заводится и осиротеть ему не с чего.
