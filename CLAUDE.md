...
- **Из перехвата выводится не весь адрес VPS, а два правила на него** - `ip daddr <VPS> tcp dport <порт туннеля> return` (без него телефон с персональным клиентом в той же LAN получил бы туннель в туннеле) и `ip daddr <VPS> tcp dport != { 80, 443 } return` (ssh и служебные порты сервера в прокси не нужны, там первым говорит сервер и соединение висит до таймаута). Под перехват остаются 80 и 443: сайты, которые живут на том же VPS, уходили из LAN голыми и провайдерский фильтр видел их SNI. Если порт туннеля выставлен в 80 или 443, правка холостая и клиент ругается в лог на старте.
