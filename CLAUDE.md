...
- **Исключение сервера в redirect сужено до порта туннеля** - правило `ip daddr <VPS> tcp dport <порт туннеля> return`, а не весь адрес VPS. Адрес целиком закрывал туннель в туннеле (телефон с персональным клиентом в той же LAN), но заодно выпускал из LAN голыми запросы к обычным сайтам, которые живут на том же VPS, и провайдерский фильтр видел их SNI.
