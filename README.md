...
- **Файлообмен** в доверенном кругу хаба: агент `xr-share` раздаёт папки и файлы, данные идут напрямую агент → потребитель, минуя хаб (см. ниже)
...
Раздача файлов «в доверенном кругу» так, чтобы **байты не проходили через хаб**.
Хаб это телефонная книга и нотариус: он знает адреса агентов и подписывает токены
доступа, но файлы потребитель забирает **напрямую у агента**. По умолчанию шара
читается; папку можно объявить писабельной (`share --writable`), и тогда держатель
инвайта с write-привязкой заливает и удаляет файлы (LLD-28).

```mermaid
flowchart LR
    AG["xr-share агент<br/>(машина владельца)<br/>раздаёт папки и файлы"]
    subgraph HubBox["Хаб (xr-hub)"]
        HK["подписывает токены"]
        IDX["индекс адресов<br/>(НЕ байты)"]
    end
    ...
    CON ==>|"4 запрос файлов напрямую"| AG
...
Текстовую шару можно править вдвоём. Владелец включает историю
(`share --writable --git`), и папка получает свой git-репозиторий, живущий вне
её самой: правка в редакторе становится коммитом сама. Соавтор держит папку в
синке одной командой `xr-share sync --invite <инвайт> --share <шара> ~/notes`,
и дальше правки едут в обе стороны без действий. Каждый работает в своём
редакторе, а git тут движок, а не инструмент: соавтору его даже не нужно
ставить. Правки в разных местах файла сливаются сами; когда обе стороны тронули
одни и те же строки, встречная версия ложится рядом отдельным файлом, и
переносить руками надо только её. Подробности в
[xr-share/README.md](xr-share/README.md) и
[docs/lld/33-share-git-sync.md](docs/lld/33-share-git-sync.md).
...
├── xr-hub/            # Централизованный контрол-плейн: пресеты, файлообмен-индекс + Admin UI
├── xr-share/          # Агент файлообмена: раздаёт папки/файлы read-only (LLD-19)
...
В `configs/routing-russia.toml` правила разложены по именованным группам: YouTube, Соцсети, Мессенджеры (с IP-диапазонами Telegram), Google, Медиа и стриминг, AI-сервисы, Разработка и IT, Приватность и VPN, Новости, Рабочие сервисы. Имя группы задаётся полем `name` у правила, оно необязательное и на маршрутизацию не влияет: его показывают приложение и админка хаба вместо счётчика доменов.
...
старте и дальше сразу, как только пресет поправили: клиент держит
висящий запрос к хабу и получает новую версию ответом на него.

Кейс: «обновил YouTube-список в хабе, и через пару секунд все 10
роутеров работают по новым правилам, никто никуда не ходил».
...
```text
       [Admin SPA / TOML-редактор]
           ...
           [xr-hub] ──── /api/v1/presets/:name
                 ▲             │
  long-poll GET  │             │ JSON + ed25519 signature
  /wait (55 с)   │             ▼
   ┌─────────────┴──────────────┐
   │           │                │
...
refresh_interval_secs = 300                    # опрос, когда хаб недоступен
```
...
Одинаково для OpenWRT-роутера (`xr-client`) и для Android-приложения
(`xr-android` поверх `xr-core`):

1. Клиент висит на `GET /api/v1/presets/<name>/wait` со своей версией
   пресета. Пока в хабе та же версия, запрос ждёт (до 55 секунд, потом
   HTTP 304 и клиент перевзводится).
2. Оператор редактирует пресет в Admin UI, хаб увеличивает версию и
   тем же ответом отдаёт ждущим клиентам полный пресет.
3. Клиент проверяет подпись и сохраняет пресет в локальный кэш
   (`/var/lib/xr-proxy/presets/<name>.json` на OpenWRT, app-private
   storage на Android).
4. **Hot-swap**: клиент пересобирает активный `Router` и заменяет его
   ...
   обновлённые правила. Рестарт службы / VPN не требуется.

То есть правило, сохранённое в Admin UI, доезжает до клиентов за
секунды. Механика одна и та же на всех клиентах, под капотом у обоих
`RwLock<Arc<Router>>` и общая фоновая задача из `xr-core`.

Если хаб не отвечает, доставка откатывается в обычный опрос: клиент
выжидает паузу (от 5 секунд с удвоением, потолок
`refresh_interval_secs`), сверяет версию, забирает изменения и пробует
снова встать на ожидание. Правила в этом режиме доезжают как раньше,
с задержкой до `refresh_interval_secs`.

### Когда хаб недоступен

Хаб — не single-point-of-failure. Клиент работает с последним
закэшированным пресетом плюс локальными override'ами. В логах WARN
вида «preset 'russia' unavailable», трафик идёт по старым правилам
без деградации.
...
В отличие от роутера, где трафик всего LAN автоматически идёт через
прокси, на Android `xr-proxy` работает как обычное VPN-приложение.
Оно поднимает локальный TUN через `VpnService`, а всю логику
(обфускация, роутинг, FakeDNS, direct-fallback) выполняет тот же
`xr-core`, что и на роутере — через JNI.

### Возможности

Те же, что у клиента на роутере, включая:

- **Централизованные пресеты** — `[hub]` работает идентично, правила
  подтягиваются с того же `xr-hub` и применяются через hot-swap
  (см. ["Как происходит обновление"](#как-происходит-обновление) выше).
- **Fallback на direct** при недоступном сервере — трафик, для которого
  не настроено проксирование, идёт мимо VPN напрямую, даже если
  VPS-сервер упал.
- **Мультирезолверный DNS** — movёк использует DNS текущей сети
  (`ConnectivityManager.LinkProperties`) плюс публичные fallback'и
  (`1.1.1.1` / `8.8.8.8` / `77.88.8.8`). Нужно для обхода ситуаций,
  когда мобильный оператор режет доступ к публичным DNS.

### Установка и сборка

Для разработческой сборки нужны Android SDK + NDK + cargo-ndk.
Скрипт `xr-android/build.sh` делает всё за один вызов:

```bash
cd xr-android
./build.sh              # debug APK
./build.sh --release    # release APK (подписан debug-ключом для теста)
```

APK появляется в `app/build/outputs/apk/{debug,release}/`. Устанавливайте
через `adb install`. Подробности — в [xr-android/build.sh](xr-android/build.sh).

### Настройка

В приложении на вкладке **Settings** ввести адрес сервера, порт,
obfuscation key и salt (те же, что в `server.toml`). Выбрать preset
или настроить свои правила. Переключиться на вкладку **VPN** и нажать
Connect — всё остальное подхватит TUN.
