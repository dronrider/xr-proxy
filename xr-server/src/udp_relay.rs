    ...
    let tx = match flows.get_mut(&key) {
        ...
        None => {
            if flows.len() >= state.max_flows {
                ...
                // пишущего до конца flow_timeout, поэтому вытесняем поток
                // с самой старой записью в туннель. Свежесть двигает только
                // этот путь, входящая ветка run_flow поле не трогает: лок
                // таблицы на каждый ответ ломал бы беслоковый цикл приёма
                // (XR-200). Поток, давно не писавший в туннель, но активно
                // принимающий, может уйти раньше своего простою. Слот уходит
                // из таблицы вместе с отправным концом очереди, таск потока
                // видит закрытый канал и выходит, забирая сокет с собой.
                let victim = flows
                    .iter()
