 ...
 *
 * `http_*`, `parse:` и `manifest_*` - категории, которыми отвечает уже сам
 * агент шары: прощуп прямого пути (XR-219) отсеивает чужой хост раньше, чем
 * ошибка доедет досюда. `humanError` этих категорий не знал, поэтому на
 ...
 * решение по существу): `5xx` или битое тело (`parse:`). Для неё имеет смысл
 * офлайн-ретрай по бэкофу, как и для настоящей недоступности сети. `404` (пути
 * нет) и `manifest_*` (подпись не сошлась, могли ответить с чужого устройства)
 * повтором не лечатся, поэтому в список не входят (XR-243).
 */
fun isTransientAgentErrorCategory(e: String): Boolean {
    val httpStatus = if (e.startsWith("http_")) e.removePrefix("http_").toIntOrNull() else null
    return e.startsWith("parse:") || (httpStatus != null && httpStatus in 500..599)
}
