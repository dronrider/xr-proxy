...
/// Маршрут публикации, каким его собирает хаб служебной ручкой
/// `POST /api/v1/web/route` для браузерного фронта (LLD-38 п. 2.3, п. 3.5).
/// Всё, что нужно посреднику, чтобы дотянуться до агента и получить право на
/// эту публикацию: сам агент (он же пин TLS), куда и с каким токеном идти
/// через relay, мандат публикации и то, сколько живёт сплайс.
///
/// Приватного ключа хаба у посредника нет и быть не должно: маршрут выдаётся
/// целиком собранным, поэтому взломанный фронт не выпишет себе мандат на
/// агента, которого хаб ему не отдавал.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct WebRoute {
    /// Имя публикации, оно же поддомен.
    pub publication: String,
    /// Base64 (standard) ed25519 ключ агента: адресат транзита и пин TLS.
    pub agent_pubkey: String,
    /// Куда и с какой обфускацией идти на relay.
    pub relay: RelayDescriptor,
    /// Транзит до агента. `share_id` у него вида `web:<имя>`
    /// (см. [`web_share_id`]), чтобы браузерный расход был отдельной строкой в
    /// агрегатах relay.
    pub relay_token: RelayToken,
    /// Мандат публикации: его агент проверяет офлайн ключом хаба.
    pub expose_token: ExposeToken,
    /// Докуда маршрут годен, unix-секунды: минимум из сроков обоих токенов.
    /// Кеш посредника живёт до этого момента минус запас.
    pub exp: u64,
    /// Потолок жизни одного сплайса на relay, секунды. Долгие соединения
    /// (WebSocket) фронт закрывает штатно до того, как relay обрубит их сам.
    pub splice_lifetime_secs: u64,
}

/// `share_id` relay-токена для публикации: `web:<имя>` (LLD-38 п. 2.5).
/// Неймспейс отдельный от идентификаторов шар, поэтому браузерный расход виден
/// в тех же агрегатах relay отдельной строкой и не смешивается с шарами.
pub fn web_share_id(publication: &str) -> String {
