...
fn extract_token_from_invite_path(path: &str) -> Result<String, InviteLinkError> {
    // Accept multiple path forms generated by xr-hub:
    //   /invite/<token>                    (canonical, LLD-04)
    //   /api/v1/invite/<token>             (API endpoint)
    //   /api/v1/invite/<token>/view        (Admin UI HTML page)
    //   /api/v1/invite/<token>/claim       (claim endpoint)
    let rest = path
        .strip_prefix("/api/v1/invite/")
        .or_else(|| path.strip_prefix("/invite/"))
        .ok_or(InviteLinkError::InvalidPath)?;
    // Strip optional trailing /view or /claim
    let token_part = rest
        .strip_suffix("/view")
...
mod tests {
    ...
    #[test]
    fn parses_api_v1_invite_path() {
        let link =
            parse_invite_link(&format!("https://hub.example.com/api/v1/invite/{TOKEN}")).unwrap();
        assert_eq!(link.hub_url(), "https://hub.example.com");
        assert_eq!(link.token(), TOKEN);
    }

    #[test]
    fn parses_api_v1_invite_view() {
        let link =
            parse_invite_link(&format!("https://hub.example.com/api/v1/invite/{TOKEN}/view"))
                .unwrap();
        assert_eq!(link.hub_url(), "https://hub.example.com");
        assert_eq!(link.token(), TOKEN);
    }
    ...
    #[test]
    fn parses_api_v1_invite_claim() {
        let link =
            parse_invite_link(&format!("https://hub.example.com/api/v1/invite/{TOKEN}/claim"))
                .unwrap();
        assert_eq!(link.hub_url(), "https://hub.example.com");
        assert_eq!(link.token(), TOKEN);
    }
    ...
    fn rejects_wrong_https_path() {
        let err =
            parse_invite_link(&format!("https://hub.example.com/other/{TOKEN}")).unwrap_err();
        assert_eq!(err, InviteLinkError::InvalidPath);
    }
