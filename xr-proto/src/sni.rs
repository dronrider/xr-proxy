...
pub fn extract_sni(buf: &[u8]) -> Option<String> {
    ...
    if hs.len() < 4 {
        // Уровень warn, а не debug: debug в бою выключен, а info прячется при
        // дефолтном log_level (он тоже warn), и срабатывание защиты осталось бы
        // невидимым. Потока из этой строки не будет, разбор зовётся один раз на
        // соединение и только на заведомо кривом вводе.
        tracing::warn!(
