...
pub fn extract_sni(buf: &[u8]) -> Option<String> {
    ...
    if hs.len() < 4 {
        // Уровень info, а не debug: в бою и на роутере, и на телефоне xr_proto
        // отфильтрован по info, а строка тут пишется только на заведомо кривом
        // вводе, поэтому потока из неё не будет.
        tracing::info!(
